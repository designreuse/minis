...
webappContent.controller('ControlitemPermissionCtrl', function($scope, $uibModal, $minis) {
    ...
    $scope.controlitemAutoComplateOptions = {
        delay : 500,
        ignoreCase : true,
        filter : "contains",
        dataTextField : "id",
        dataSource : {
            ...
            transport: {
                read : {
                    ...
                    url : Minis.getBaseUrl("~/aa/autocomplate/controlitem"),
                },
        ...
        change: function() {
            $scope.controlitem = this.dataItem();
        },
    };
    ...
    $scope.findControlitemPermissions = function() {
        ...
        if ($scope.controlitem == undefined) {
            return;
        }

        $.blockUI();

        var url = "~/aa/controlitem/permission/findByControlitemId/" + $scope.controlitem.id;
    ...
    $scope.saveControlitemPermissions = function() {
        ...
        var url = "~/aa/controlitem/permission/updateByControlitemId/" + $scope.controlitem.id;
